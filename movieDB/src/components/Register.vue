<template>
    <div class="form-register">
        <form>
            ...
            <div class="input-container">
                <label for="repassword" class="label-login">Verify password</label>
                <input type="password" class="input-login" name="repassword" v-model="rePassword" @blur="$v.rePassword.$touch()" :class="{invalid: $v.rePassword.$error}">                      
                <p class="error-message" v-if="$v.rePassword.$error">Passwords must be identical</p>
            </div>
            <div class="input-container">
                <input type="checkbox" name="terms" id="terms" class="input-login-checkbox" v-model="terms" @change="$v.terms.$touch()">
                <label for="terms" class="label-login-checkbox" :class="{invalidCheck: $v.terms.$error}">                    
            ...
            <div class="input-container">
                <input type="submit" value="Register" class="login-submit" @click.prevent="submitRegister" :disabled="$v.$invalid">                    
            </div> 
        </form>
    </div>
</template>
...
export default {
    ...
    methods: {
        submitRegister() {
            ...
            axios.post(`${this.baseURL}/users`, encodeForm(Payload), {headers: {'Accept': 'application/json'}})
            .then(res => {
                window.localStorage.setItem('authToken', res.headers['x-auth-token']);
                window.localStorage.setItem('username', res.data.CREATE_USER_PAYLOAD.username);
