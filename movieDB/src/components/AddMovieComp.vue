                ...
                <div class="flex-start">
                    ...
                    <div class="col-2">
                        ...
                        <div class="input-divider">
                            ...
                            <input type="number" class="input-movie" name="votes" v-model="votes" @blur="$v.votes.$touch()" :class="{borderError: $v.votes.$error}">  
                        ...
                        <div class="flex-end">
                            <button type="submit" @click.prevent="submit" class="btn-submit" :disabled="$v.$invalid">Save</button>
                        </div>
                    </div>
                </div>
            </form>
    ...
    methods: {
        async getMovieToEdit(id) {
            await axios.get(`${this.baseURL}/movies/${id}`).then(response => {
                this.title = response.data.movies.Title;
                this.year = response.data.movies.Year;
                this.runtime = response.data.movies.Runtime;
                this.genre = response.data.movies.Genre;
                this.country = response.data.movies.Country;
                this.poster = response.data.movies.Poster;
                this.language = response.data.movies.Language;
                this.rating = response.data.movies.imdbRating;
                this.votes = response.data.movies.imdbVotes;
                this.imdbId = response.data.movies.imdbID;
                this.type = response.data.movies.Type;
                });            
        },        
        submit() {
            const getHeaders = {
            ...
            if(this.id) {
                axios.patch(`${this.baseURL}/movies/${this.id}`, encodeForm(Payload), {headers: getHeaders})
                .then(res => {
                ...
                axios.post(`${this.baseURL}/movies`, encodeForm(Payload), {headers: getHeaders})
                .then(res => {
                    this.snackbarAdd = true;
                    setTimeout(() => this.$router.push(`/`),1000);                    
                })
